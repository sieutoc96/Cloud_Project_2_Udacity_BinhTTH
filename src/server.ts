...
(async () => {
  ...
  app.get("/filteredimage", async (req: any, res: any) => {
    ...
    if (!imageUrl) {
      return res.status(400).send('image url is required, try agian !');
    }

    const regex = /(?:https?):\/\/(\w+:?\w*)?(\S+)(:\d+)?(\/|\/([\w#!:.?+=&%!\-\/]))?/;
    const imageUrlRegex = /\.(jpeg|jpg|gif|png|webp)$/i;
    if (!regex.test(imageUrl) || !imageUrlRegex.test(imageUrl)) {
      return res.status(400).send('image url is invalid, try agian !');
    }
